...
public:
	 ...
	 */
	bool begin(SpiMaster* spi, uint32_t speed);
	 ...
	 */
	void memBurstWriteBytes(uint32_t memAddress, const void* memValues, uint16_t count, SpiCallback callback = nullptr,
							void* param = nullptr);
	 ...
	 *
	 */
	void memBurstReadBytes(unsigned int memAddress, void* memValues, uint16_t count, SpiCallback callback = nullptr,
						   void* param = nullptr);
...
private:
	 ...
	 *   then copied to the regInitValues[] sequence in the
	 *   S1d13781_init.h file.
	 *
	 */
	bool initRegs();
	...
	SpiDevice spidev; ///< SPI device to talk to display
